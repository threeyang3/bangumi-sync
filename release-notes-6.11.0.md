...
6.11.0 发布后确认了七个实现缺陷：可解析但结构不完整的 schema-1 journal 会中断启动；部分 recovery reason 可被空 Retry 错误清除；rename 与内容恢复顺序需要第二次 Retry；图片质量与已有图片更新设置未完整接线；已有封面更新不可回滚；提交成功未向订阅者广播 idle；长期打开的设置页可能覆盖运行期新设置。上述问题属于 6.11.0 的已发布历史事实，并由 6.11.1 修复；本文件其余发布内容保持原样。
