...
- **同步状态拆分**：控制面板和命令入口现在分别提供“同步用户数据”和“同步平台数据”，不再把两类决策混在同一个入口里
- **移除本地连载状态字段**：默认模板和 frontmatter 平台写回不再使用 `连载状态` 属性，平台同步不再依赖本地完结判定做跳过
- **平台同步逻辑收口**：平台数据仍以云端为准，但只同步平台字段本身，不再尝试通过本地 `连载状态` 属性做状态修正
- **测试与模块化清理**：同步逻辑、模板、frontmatter 读写和平台字段测试已同步更新，避免旧设计残留
