...
- **同步状态统一入口**：控制面板恢复为单一“同步状态”按钮，先弹出字段范围选择窗口，再进入差异对比
- **字段范围可配置**：同步状态支持按“用户数据 / 平台数据”两大栏勾选本轮要检索和对比的字段，默认只勾选用户数据
- **控制面板工具栏重构**：默认态与选中态分别保持固定 8 键布局，移动端继续 4x2，桌面端继续单行
- **旧同步功能残留清理**：移除旧的短评同步、标签同步、平台专用状态同步入口及相关残留翻译/预设代码，状态同步逻辑进一步收口
