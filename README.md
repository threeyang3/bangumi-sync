...
| 评分 | 观看状态 | 我的评分 |
|------|----------|----------|
| `= this.rating` | `= this.观看状态` | `= this.my_rate` |
```
...
### 模板系统优化
- **两套模板**：新增标准模板（只含 Bangumi 数据）和作者自用模板（含自定义变量）
- **响应式布局**：bangumi-info callout 支持宽屏并排显示、窄屏上下排列
- **笔记路径模板**：支持自定义笔记链接路径，如 `ACGN/笔记/`
- **动画公司获取**：改进动画公司字段获取，支持从 Copyright 字段提取
- **模板文档**：更新模板设计指南，添加自定义变量说明

### v4.3.1 模板优化
- **移除别名属性**：移除所有模板中的"别名"属性
- **优化属性顺序**：用户个人数据移到 Bangumi 评分和链接前面
- **漫画模板优化**：移除冗余的"连载状态"属性
- **新增记录感想**：小说、漫画、游戏、画集模板添加 ##记录、##感想 部分
- **动画公司获取**：扩展备选字段名，提高获取成功率
