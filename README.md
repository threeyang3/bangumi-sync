...
- **状态同步提速**：改为“快速首屏 + 后台补全”，首屏优先展示用户数据差异，平台数据与单集状态后台增量加载
- **控制面板预热缓存**：打开控制面板后后台预热已同步条目的本地用户数据，状态同步时避免重复读取正文
- **平台数据同步增强**：动画/三次元/小说/漫画支持按未完结候选条目比较集数、话数、卷数/册数，并在云端已完结时联动修正本地状态和计数
- **短评 callout 统一**：导入、同步、模板渲染统一使用正文 `> [!abstract]+ **短评**`，多段短评读写保持一致
