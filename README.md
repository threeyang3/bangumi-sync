...
- **新增 `{{platform}}` 变量**：可直接使用 Bangumi API 的平台/具体类型字段（如"公式书"、"TV"、"电影"），适用于模板内容和路径模板
- **各类型独立路径模板**：允许每种条目类型分别设置同步到本地的路径
- **三次元类型增强**：新增作者自用模板，包含集数展示、记录、感想部分
- **已同步条目更新修复**：再次同步时 API 衍生属性（如具体类型）会正确更新
