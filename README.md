...
- **状态同步服务进一步拆分**：将状态同步内部明确拆成快照构建、后台补全、执行写回和门面服务，降低控制面板与弹窗的业务负担
- **本地快照会话缓存**：控制面板预热、状态同步和批量编辑开始共享一轮会话内的本地条目快照，减少重复读取和重复归一化
- **批量编辑 frontmatter 逻辑收口**：frontmatter 的解析、显示值格式化和草稿值转换统一收口到文档层，减少重复实现
- **逐项编辑显示修复**：修复批量编辑逐项编辑模式下普通字符串 frontmatter 属性可能显示为空的问题
