...
| 设置项 | 说明 |
|--------|------|
| 保留评分明细 | 强制同步时保留评分明细类自定义属性 |
| 保留自定义属性 | 强制同步时保留其他自定义属性 |
| 保留记录 | 强制同步时保留正文中的记录 |
| 保留感想 | 强制同步时保留正文中的感想 |
...
- 推荐把评分明细也写成普通 frontmatter 属性，例如：
...
- 旧模板中的 `{{rating_story}}`、`{{rating_music}}` 等写法仍兼容，但新的内置作者模板已经改为普通属性写法
...
- 修复同步到本地前拉取条目详情时，单个请求失败会导致整批同步中断的问题；现在会回退到收藏列表中的简版条目信息继续完成自定义属性填写
- 优化自定义属性填写窗口布局与宽度，避免 Obsidian 默认 modal 样式覆盖，短字段会更紧凑地并排显示
- 自定义属性链路继续保持统一：同步收藏、控制面板同步、搜索并添加、导入导出、强制同步继承都走同一套模板属性分类逻辑
...
### 修复
- **收藏状态查询错误**：修复 `/collections/...` 404 刷屏与已收藏状态误判
- **动态属性漏判**：修复 `资源属性` 等字段被模板条件块误判而不出现在填写窗口的问题
- **数组型属性导入退化**：修复列表型自定义属性在导入、覆盖、继承时被错误当成字符串处理的问题
