...
- **取消回滚**：取消同步后，本次新建的文件会被移到回收站（已覆盖更新的文件不受影响）
...
- 对比本地与云端差异
- 用户数据与平台数据分开处理：评分 / 短评 / 标签 / 收藏状态 / 单集状态全量对比，平台集数/话数/卷数先排除本地明确已完结条目
- 选择保留本地版本或云端版本
- 支持智能合并（标签）
- 批量处理多个条目
- 单集观看状态同步
...
- **文档服务层收口**：统一抽出本地条目文档服务，短评、frontmatter、`## 记录 / 感想`、平台字段和单集状态快照不再由多个功能各自维护字符串读写
- **状态同步 service 化**：状态同步的初始 diff、后台补全、默认决策和执行写回已从 UI 层下沉到独立 service，`ControlPanel` 与 `StatusSyncModal` 只保留编排和展示职责
- **规范对齐**：补齐文档层与状态同步层测试，修正 `statusSyncModal` 的全局 `window` 回退，继续符合 Obsidian 的 popout window 兼容规范
- **测试护栏增强**：`Vitest` 新增 frontmatter 访问和 markdown section 读写覆盖，进一步守住导入、状态同步和正文更新回归风险

> 说明：Bangumi OpenAPI 虽然提供 `updated_at`，但官方说明该字段不会可靠反映评分、短评、标签、章节状态等收藏修改，因此插件当前不使用它作为“用户数据未变化”的跳过依据，而是采用安全的本地预热缓存与本轮内存缓存。
