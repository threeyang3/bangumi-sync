...
- **连载状态修复**：动画和三次元平台数据解析现在兼容 `播放开始 / 播放结束` 字段，避免已完结作品在状态同步中被误判为 `连载中`
- **平台字段默认云端权威**：平台数据差异现在默认选择 `cloud`，执行状态同步时会按云端修正 `连载状态 / 结束 / 进度 / 集数` 等字段
- **代码质量优化**：抽出平台字段 diff helper，减少状态同步 service 中重复对象拼装
- **测试护栏增强**：新增 Bangumi API 字段形态回归测试和平台字段默认决策测试，纯逻辑测试扩展到 30 项
