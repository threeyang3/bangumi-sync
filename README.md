...
### 改进
- **状态文案统一**：本地模板、控制面板、搜索弹窗、状态同步弹窗统一使用按条目类型生成的收藏状态文案
- **状态风格一致**：想看、看过、在看、搁置、抛弃五种状态现在都使用统一的“文字 + 图标”风格，并区分看/读/玩/听

### 修复
- **双向同步兼容性**：本地状态写回 frontmatter 后仍可被状态同步正确识别，不会因为状态符号导致解析失败
