...
### 修复
- **状态同步与单条同步**：修复部分 Bangumi `POST /collections` 成功但空响应体时被误判为失败的问题
- **用户数据导出**：修复“记录 / 感想”多段正文只导出首行的问题
- **导出筛选逻辑**：改为基于“全部 frontmatter 属性减去 Bangumi 可重建字段”的负面过滤，兼容自定义模板属性

### 改进
- **代码结构**：收敛 frontmatter 解析、定时器和用户数据导出逻辑中的重复实现
- **Obsidian 兼容性**：清理设置页 inline style，并改进外链打开方式与弹窗样式作用域
