...
- **API 重试机制**：请求遇到 429 或 5xx 错误时自动重试（指数退避）
- **错误详情展示**：同步完成后可折叠查看每个失败条目的具体错误信息
- **同步并发数可设置**：设置页新增并发数滑块（1-5），控制同时处理的条目数量
- **并发同步统一**：控制面板同步和手动同步均已支持并发处理
- **性能优化**：双向链接批量更新、路径查找索引、分页动态间隔
- **新增 11 个模板变量**：`from`、`musicMake`、`audioDirector`、`artDirector`、`animeChief`、`start`、`end`、`script`、`art`、`producer`、`price`
