...
export class StatusSyncScopeModal extends Modal {
	...
	private renderSection(container: HTMLElement, section: 'user' | 'platform'): void {
		...
		for (const field of fieldDescriptors) {
			const isChecked = section === 'user'
				? this.selection.user[field.key as (typeof USER_STATUS_SYNC_FIELD_KEYS)[number]]
				: this.selection.platform[field.key as (typeof PLATFORM_STATUS_SYNC_FIELD_KEYS)[number]];
			...
			checkbox.addEventListener('change', () => {
				if (section === 'user') {
					this.selection.user[field.key as (typeof USER_STATUS_SYNC_FIELD_KEYS)[number]] = checkbox.checked;
				} else {
					this.selection.platform[field.key as (typeof PLATFORM_STATUS_SYNC_FIELD_KEYS)[number]] = checkbox.checked;
				}
				this.render();
			});
		...
		const selectedCount = fieldDescriptors.filter(field => {
			if (section === 'user') {
				return this.selection.user[field.key as (typeof USER_STATUS_SYNC_FIELD_KEYS)[number]];
			}
			return this.selection.platform[field.key as (typeof PLATFORM_STATUS_SYNC_FIELD_KEYS)[number]];
		}).length;
