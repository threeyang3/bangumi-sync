...
import { BangumiClientV2 } from '../api/client';
import { Subject, UserCollection, SubjectType, CollectionType } from '../../../common/api/types';
import { FileManagerV2 } from '../file/fileManager';
import { ImageHandler } from '../../../common/file/imageHandler';
import { IncrementalSyncV2 } from './incrementalSync';
import { SyncOptionsV2, SyncResultV2, SyncProgressV2 } from './syncStatus';
...
import { generateContentByTypeV2 } from '../template/contentTemplate';
 ...
 * 同步管理器配置
 */
export interface SyncManagerConfigV2 {
	accessToken: string;
	pathTemplate: string;
...
export class SyncManagerV2 {
	private app: App;
	private client: BangumiClientV2;
	private fileManager: FileManagerV2;
	private imageHandler: ImageHandler;
	private incrementalSync: IncrementalSyncV2;
	private config: SyncManagerConfigV2;
	private onProgress?: (progress: SyncProgressV2) => void;

	constructor(app: App, config: SyncManagerConfigV2) {
		this.app = app;
		this.config = config;
		this.client = new BangumiClientV2(config.accessToken);
		this.fileManager = new FileManagerV2(app);
		this.imageHandler = new ImageHandler(app, this.fileManager as any);
		this.imageHandler.setDownloadEnabled(config.downloadImages);
		this.incrementalSync = new IncrementalSyncV2(app);
	}

	/**
	 * 设置进度回调
	 */
	setProgressCallback(callback: (progress: SyncProgressV2) => void): void {
	 ...
	 * 更新配置
	 */
	updateConfig(config: Partial<SyncManagerConfigV2>): void {
	 ...
	 * 执行同步
	 */
	async sync(options: SyncOptionsV2): Promise<SyncResultV2> {
	...
	private async processCollection(collection: UserCollection): Promise<void> {
		...
		const content = generateContentByTypeV2(
			subject,
			collection,
			characters,
	 ...
	 * 报告进度
	 */
	private reportProgress(progress: Partial<SyncProgressV2>): void {
		if (this.onProgress) {
			this.onProgress({
				current: 0,
				total: 0,
	 ...
	 * 用于手动同步模式，在显示预览弹窗前调用
	 */
	async prepareSync(options: SyncOptionsV2): Promise<{
		success: boolean;
