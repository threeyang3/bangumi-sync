...
> 历史文档：本文记录特定版本的验证事实，不是当前回归模板。当前模板见 [`docs/maintainer/REGRESSION_CHECKLIST.md`](../../maintainer/REGRESSION_CHECKLIST.md)。
...
历史检查重点包括 absent/present 身份矩阵、重复 ID、temporary file、CRLF/LF、scan root 外路径、相关链接延迟提交、结果统计及所有 Vault 写入口门禁。当前发布验收模板以[维护者回归清单](../../maintainer/REGRESSION_CHECKLIST.md)为准。
