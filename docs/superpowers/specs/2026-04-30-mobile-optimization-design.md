...
截至 `v5.3.4`，`mobile` 分支已经完成控制面板移动端优先优化，并发布同名 prerelease 供 BRAT 测试。已落地内容包括：
...
- 搜索弹窗在移动端使用单行搜索、单行筛选和带封面的紧凑结果卡片，添加/编辑按钮放在元数据行右侧
- 搜索结果标题独占内容行，避免按钮挤占标题宽度
- 控制面板关闭按钮避开 iOS 顶部状态栏安全区
- 滑动关闭只在列表滚动到顶部时启用，减少误触
- 移动端检测只使用 `max-width: 767px` 断点，不再把触摸屏桌面设备误判为移动端

未完整落地的原始设想仍包括：状态同步弹窗折叠列表、设置页分组折叠。搜索弹窗已做移动端紧凑布局，但尚未加入筛选栏折叠开关。这些内容仍可作为后续计划，但不要按本文旧草案误判为已经全部完成。
...
| `src/panel/statusSyncModal.ts` | 待后续确认 | 原计划添加折叠列表支持，v5.3.4 尚未完整实现 |
| `src/ui/searchModal.ts` | 修改 | 移动端搜索栏、筛选栏和结果卡片已紧凑化；筛选栏折叠开关尚未完整实现 |
| `src/ui/localPropertyModal.ts` | 修改 | 移动端布局适配 |
| `src/ui/addToCollectionModal.ts` | 修改 | 移动端布局适配 |
| `src/settings/settingsTab.ts` | 待后续确认 | 原计划添加设置分组折叠，v5.3.4 尚未完整实现 |
