...
截至 `v5.8.0`，移动端适配已全面完成并达到生产级标准。已落地内容包括：

- **控制面板（生产级）**：
  - 条目列表采用卡片式布局，左侧操作列，右侧内容区。
  - 底栏重构：状态文本（加载中/已同步）与分页信息（页码/总数）在中间列垂直堆叠，翻页按钮跨两行垂直居中。
  - 顶部操作栏压缩为 4 列网格，按钮标签适配移动端。
- **搜索弹窗（生产级）**：
  - 布局严格参考 `preview-v2.html`：第一行为搜索框、搜索按钮、清空按钮；第二行为类型选择器和排序选择器。
  - 结果卡片带封面缩略图，标题独占行，元数据与添加按钮同排。
- **全局适配**：
  - 移动端检测断点统一为 `768px`。
  - 增加 iOS 安全区适配及滑动关闭手势。
  - 清理了所有冗余的移动端媒体查询样式。
