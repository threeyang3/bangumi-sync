...
**Current status (2026-05-02):** 本文件是原始执行计划，不再是完全准确的待办清单。`v5.3.4` 已完成控制面板移动端卡片化、紧凑工具栏、状态栏/分页压缩、自定义勾选样式、iOS 安全区关闭按钮、顶部滚动下拉关闭防误触，以及搜索弹窗移动端紧凑布局，并发布 `5.3.4` prerelease 供 BRAT 测试。搜索弹窗筛选栏折叠开关、状态同步折叠列表、设置页分组折叠仍未完整实现，不能按下方自检清单视为已完成。
