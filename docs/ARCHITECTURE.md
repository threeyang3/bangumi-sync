...
- `src/utils/`
  ...
  - 移动端 UI 核心逻辑：控制面板使用 Grid 底栏实现状态与分页同列对齐；搜索弹窗引入 `bangumi-search-selectors-container` 实现两行式布局，并优化了标题层级（中文加粗，原名淡化）。
  - 桌面端 UI 核心逻辑：弹窗采用 Flex 纵向布局配合 `overflow: hidden` 消除双滚动条；通过 `color-mix` 统一所有输入控件与面板背景的色调，实现全局“纸张感”视觉。
