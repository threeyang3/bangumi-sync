...
只包含 Bangumi 数据，适合普通用户：
- 所有数据从 Bangumi API 获取
- 不包含自定义字段
- 简洁清晰，开箱即用
...
| 来源 | 说明 |
|------|------|
| 标准模板 | 使用插件内置的标准模板（只含 Bangumi 数据） |
| 作者自用模板 | 使用插件内置的作者自用模板（含自定义变量） |
...
以下变量无法从 Bangumi API 获取，需要用户手动填写：

### 动画自定义变量

| 变量 | 说明 | 用途 |
|------|------|------|
| `标语` | 作品标语/宣传语 | 记录印象深刻的台词 |
| `评分明细` | 各维度评分 | 音乐、人设、剧情、美术 |
| `单评` | 是否单集评论 | 标记评论类型 |
| `精彩片段` | 精彩片段记录 | 记录喜欢的场景 |
| `存储` | 存储位置 | 本地/网盘等 |
| `资源属性` | 资源属性 | 1080p/4K 等 |
| `相关` | 相关作品 | 双向链接 |
...
|------|------|------|
| `标语` | 作品标语 | 记录印象深刻的句子 |
| `评分明细` | 各维度评分 | 剧情、插画、文笔、人设 |
| `单评` | 是否单卷评论 | 标记评论类型 |
| `版本` | 版本信息 | 繁简、特典等 |
| `Kindle` | 是否有 Kindle 版 | true/false |
| `保存` | 是否保存 | true/false |
| `相关` | 相关作品 | 双向链接 |
| `渠道` | 购买渠道 | 实体/电子等 |
| `已购` | 是否已购买 | true/false |
...
|------|------|------|
| `标语` | 作品标语 | 记录印象深刻的台词 |
| `评分明细` | 各维度评分 | 剧情、趣味、音乐、美术 |
| `单评` | 是否单关评论 | 标记评论类型 |
| `存储` | 存储位置 | Steam/Epic 等 |
| `相关` | 相关作品 | 双向链接 |
| `平台` | 游玩平台 | PC/Switch/PS5 等 |
